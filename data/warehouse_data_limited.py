"""
Depo İş Emirleri ve Operatör Verileri - KISITLI KAPASİTE SENARYOSU
4 operatör aktif, 4 operatör izinli/hasta
"""
import pandas as pd
from typing import Tuple


def get_warehouse_data_limited() -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """
    SENARYO 2: Kısıtlı kapasite - sadece 4 aktif operatör
    
    Returns:
        Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]: 
            - tur_info: İş emirleri (IsEmri, UrunDesi, UrunAdedi)
            - df_operator: Operatör listesi
            - operator_status: Operatör durumları (4 aktif, 4 izinli/hasta)
    """
    ...
    # ⚠️ DEĞİŞİKLİK: Sadece 4 operatör aktif (4 izinli/hasta)
    operator_status = pd.DataFrame({
        'Operator': operator_list,
        'Status': ['active', 'active', 'on_leave', 'on_leave', 'active', 'sick_leave', 'sick_leave', 'active'],
        'WeeklyHours': [42, 38, 0, 0, 40, 0, 0, 35],
...
if __name__ == "__main__":
    # Test
    tur_info, operators, status = get_warehouse_data_limited()
    print("📦 İş Emirleri:")
    print(tur_info.head())
    ...
    print("\n👷 Operatör Durumu:")
    print(status)
    print(f"\nAktif: {len(status[status['Status'] == 'active'])} operatör")